<?php
    ...
    function displayfilelist($message = ""){
        ...
        $form .= "<input type=\"text\" name=\"filename\" id=\"filename\" placeholder=\"(Ingrese el nombre del archivo)\" maxlength=\"100\" />\n";
        $form .= "</div>\n</li>\n";
        $form .= "<li>\n<div id=\"boton\">\n";
    ...
    function displayeditform($filename = ""){
        ...
        $editform .= "\t\t\t<textarea name=\"filecontents\" id=\"filecontents\" cols=\"80\" rows=\"20\">\n";
    ...
    function savefile(){
        ...
        if(file_exists($filepath)){
            ...
            if(file_put_contents($filepath, $_POST['filecontents']) === false)die("<p class=\"error\">No se ha podido guardar el archivo</p>\n");
            displayfilelist();
        }else{
    ...
    function createfile(){
        ...
        if(!$filename){
            displayfilelist("<p class=\"error\">Nombre de archivo no v&aacute;lido. Pruebe con otro nombre.</p>");
            return;
        }
        $filename .= ".txt";
        $filepath = PATH . "/$filename";
        if(file_exists($filepath)){
            ...
            if(file_put_contents($filepath, "") === false) die("No se ha podido crear el archivo");
            chmod($filepath, 0777);
