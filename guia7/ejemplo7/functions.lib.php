<?php
    function showform(){
        ...
        <input type="text" name="firstname" id="namanyay-search-box" placeholder="(Nombre a buscar)"/>
...
function processfile(){
    ...
    foreach($rows as $register):
        ...
        if(strcasecmp($fullname[0], $firstname) == 0):
            ...
            $newstring = implode("<br />", array($fields[0], $phone, $address, implode("/", $birthday), '$ ' . number_format(floatval($salary), 2, '.', ',')));
            echo "<p>" . $newstring . "</p>";
            $count++;
        endif;
    endforeach;
