(function() {
    ...
    function init() {
        var number1 = document.getElementById("nota1");
        var number2 = document.getElementById("nota2");
        var number3 = document.getElementById("nota3");
        var number4 = document.getElementById("nota4");
        var number5 = document.getElementById("nota5");
        ...
        if (number4.addEventListener) {
            number4.addEventListener("keypress", filter, false);
        } else {
            // No utilizamos attachEvent porque no llama a la función de controlador 
            // con el valor correcto de la palabra clave this. En su lugar se invova
            // con el manejador de evento onkeypress  
            number4.onkeypress = filter;
        }
        if (number5.addEventListener) {
            number5.addEventListener("keypress", filter, false);
        } else {
            // No utilizamos attachEvent porque no llama a la función de controlador 
            // con el valor correcto de la palabra clave this. En su lugar se invova
            // con el manejador de evento onkeypress  
            number5.onkeypress = filter;
        }
    }
    ...
    function filter(event) {
        ...
        var messageElement4 = document.getElementById("numbersOnly4"); // Mensaja a ocultar/mostrar
        var messageElement5 = document.getElementById("numbersOnly5"); // Mensaja a ocultar/mostrar
        ...
        if (allowed1.indexOf(c) != -1) {
            // Si c es un carácter legal, ocultar el mensaje, si es que existe.
            if (messageElement4) messageElement4.style.visibility = "hidden";
            return true; // Aceptar el carácter
        } else {
            // Si c no está en el conjunto de caracteres permitidos, mostrar el mensaje
            if (messageElement4) messageElement4.style.visibility = "visible";
            // Y rechazar este evento keypress
            if (e.preventDefault) e.preventDefault();
            if (e.returnValue) e.returnValue = false;
            return false;
        ...
        if (allowed1.indexOf(c) != -1) {
            // Si c es un carácter legal, ocultar el mensaje, si es que existe.
            if (messageElement5) messageElement5.style.visibility = "hidden";
            return true; // Aceptar el carácter
        } else {
            // Si c no está en el conjunto de caracteres permitidos, mostrar el mensaje
            if (messageElement5) messageElement5.style.visibility = "visible";
            // Y rechazar este evento keypress
            if (e.preventDefault) e.preventDefault();
            if (e.returnValue) e.returnValue = false;
            return false;
