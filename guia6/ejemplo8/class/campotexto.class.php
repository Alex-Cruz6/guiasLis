<?php
    //Clase abstracta para algún tipo de campo deformulario
    ...
    class campotexto extends campoformulario {
        ...
        function pinta_campo(){
            ...
            $stag .= "\t\t<i class=\"icon-prepend icon-user\"></i>\n";
                $stag  .=  "\t\t<input type=\"text\" name=\"" . $this->idcampo . "\" id=\"".  $this->idcampo  .  
                    "\"  placeholder=\""  .  $this->placeholder  .  "\" maxlength=\"" . $this->maxcar . "\"" . 
    ...
    class campotextarea extends campoformulario {
        ...
        function __construct($id, $etiq, $lineas, $cols, $ayuda, $placehold){
            parent::__construct($id, $etiq, $ayuda);
            $this->placeholder = $placehold;
            $this->lineas = $lineas;$this->cols = $cols;
        }
        function pinta_campo() {
            $tag  = "\t<section>\n";
            $tag  .=  "\t\t<label  for=\"$this->idcampo\"  class=\"textarea\">$this->etiqueta\n";
            $tag .= "\t\t<textarea name=\"" . $this->idcampo . "\" id=\"" . $this->idcampo . "\" rows=\"" . 
                $this->lineas . "\" cols=\"" . $this->cols . "\" " . $this->poner_eventos_js() . ">";
    ...
    class campocheckbox extends campoformulario {
        ...
        function pinta_campo(){
            $pos = 0;  
            //Indica la posición en el arreglo de opciones del checkbox
            ...
            foreach($this->options['opciones'] as $key => $value){
                ...
                $stag  .=  "\t\t<input  type=\"checkbox\"  value=\"$value\"  
                    name=\"$this->idcampo" . $pos . "\" id=\"$this->idcampo" . $pos . "\" ";
    ...
    class camposelect extends campoformulario{
        ...
        function pinta_campo(){
            ...
            $seltag .= "\t\t<select name=\"" . $this->idcampo . "\" id=\"" . $this->idcampo . "\" size=\"" . 
                $this->size . "\"" . $this->poner_eventos_js() . ">\n";
    ...
    class camposubmit extends campoformulario{
        ...
        function pinta_campo(){
            $subtag  = "<input type=\"submit\" name=\"" . $this->idcampo . "\" id=\"" . 
                $this->idcampo . "\" value=\"" . $this->value . "\" class=\"button\"";
